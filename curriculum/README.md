...
carry no sequence (naming rule). **Tracks A and B are written; C, D, E and F are not
yet.**

Every `file:line` pointer below is machine-verified (`scripts/verify_code_pointers.py`,
260/260 resolving) and every arXiv id resolved against the live API
(`scripts/verify_citations.py`, 147/147). See `citation-verification.json`.
