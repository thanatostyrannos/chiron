...
| gpu-fast-tier-size | Capacity is not uniform: only a ~30 GiB working set runs at full bandwidth, and beyond it bandwidth halves. This — not the 82.67 GiB ceiling — is the number that sizes KV-cache experiments. | **untested (mechanism)** | `[M]` 2026-07-26, `scripts/measure_memory_bandwidth_tiers.py`, single run: 185–210 GB/s up to a 30 GiB footprint, 61 GB/s at 32 GiB, 92–114 GB/s beyond. Sharp boundary, ~2x effect, does **not** align with the 16 GiB dedicated carve-out. Test: `notebook/uma-carveout-controls-fast-tier.md` — raise BIOS UMA to 96 GB and re-run the identical probe. | 2026-07-26 |
...
- ~~**`hardware-capacity-ceiling` is unmeasured.**~~ Filled 2026-07-26: **82.67 GiB**
  ...
  native Windows.
- **The number that actually binds is smaller than the ceiling.** Only a ~30 GiB
  working set runs at full bandwidth (`gpu-fast-tier-size`). Long-context experiments
  should be budgeted against 30 GiB, not 82.67 GiB, until
  `notebook/uma-carveout-controls-fast-tier.md` resolves whether the boundary moves.
